...
"""
        ...
        # Handle default employee location sharing (from tracking system)
        employee_name = None
        for name, chat_id in EMPLOYEES.items():
            if chat_id == message.chat.id:
                employee_name = name
        ...
        if employee_name:
            # This is an employee sending location, handle it
...
"""
    ...
    def select_task_employee(message):
        ...
        if message.text in EMPLOYEES:
            ...
            if "description" not in data or "location" not in data:
                bot.send_message(
                    message.chat.id,
                    "❌ Vazifa ma'lumotlari to'liq emas. Qaytadan boshlang."
                )
                clear_user_state(message.chat.id)
                show_admin_panel(message)
                return
            ...
            task_id = add_task(
                ...
                payment_amount=data.get("payment", 0),
...
"""
        ...
        keyboard.add(
            ...
            types.InlineKeyboardButton("📱 Telefondan ochish", url=f"https://maps.google.com/?q={latitude},{longitude}")
        )
        keyboard.add(
